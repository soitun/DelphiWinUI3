...
uses
  System.IOUtils, System.Win.Registry, Winapi.CommDlg, Winapi.ShlObj, Winapi.MMSystem, Winapi.ShellAPI,
  ...
  FMX.Platform.Metrics, FMX.Platform.Metrics.Win, FMX.VirtualKeyboard.Win, FMX.Platform.Timer.Win,
  FMX.Platform.Logger.Win, FMX.Platform.Menu.Win, FMX.FontManager, FMX.FontManager.Win;
...
type
  ...
  TFormInfo = class
    ...
    AdditionalMouseShiftState: TShiftState;
  end;
  ...
  private
    ...
    FRunning: Boolean;
    FCursor: TCursor;
    FCaptionChangedId: TMessageSubscriptionId;
...
{ Text Service }
  ...
  TWinSystemAppearanceService = class(TInterfacedObject, IFMXSystemAppearanceService)
  const
    DefaultAccent = $FF0078D7;
  private type
    TSystemColors = array [TSystemColorType] of TAlphaColor;
  private
    FSystemColor: TSystemColors;
  ...
  protected
    ...
    function ReadThemeKindFromRegistry: TSystemThemeKind;
    function ReadSystemColorFromRegistry(const AType: TSystemColorType): TAlphaColor;
  public
    constructor Create;
    destructor Destroy; override;
...
begin
  ...
  TPlatformServices.Current.AddPlatformService(IFMXFormPositionerService, TWinFormPositionerService.Create);

  MenuServiceWin := TWinMenuService.Create;
  TPlatformServices.Current.AddPlatformService(IFMXMenuService, MenuServiceWin);

  TPlatformServices.Current.AddPlatformService(IFMXFontManagerService, TWinFontManager.Create);
  TPlatformServices.Current.AddPlatformService(IFMXSystemAppearanceService, TWinSystemAppearanceService.Create);
...
begin
  ...
  begin
    ...
    TPlatformServices.Current.RemovePlatformService(IFMXFormPositionerService);
    ...
    TPlatformServices.Current.RemovePlatformService(IFMXPlatformPropertiesService);
...
begin
  ...
  FreeAndNil(FMultiTouchManager); // release multitouch early so that it does not hold reference to services
  ...
  begin
    ...
    try
      ...
      begin
        ...
        Form.RecreateResources;
        Form.RecreateOsMenu;
        TMessageManager.DefaultManager.SendMessage(nil, TScaleChangedMessage.Create(Form), True);
      end;
    except
      Application.HandleException(Self);
    end;
          ...
          begin
            ...
            try
              ...
              begin
                ...
                for I := 0 to Integer(RegionData.rdh.nCount) - 1 do
  ...
  if GetTouchInputInfo(TouchInputInfo, TouchCount, @Inputs[0], SizeOf(TTouchInput)) then
    try
      ...
      begin
        ...
        Touches[I].Id := NativeInt(Input.dwID);
        Touches[I].Location := AForm.ScreenToClient(ScreenPoint);
        Touches[I].Action := Action;
      end;
      ...
      PlatformWin.FMultiTouchManager.EnabledInteractiveGestures := PlatformWin.FEnabledInteractiveGestures;
...
type
  TMouseButtonHelper = record helper for TMouseButton
    constructor Create(const AMessage: UINT);
    function ToShiftState: TShiftState;
  end;

{ TMouseButtonHelper }

constructor TMouseButtonHelper.Create(const AMessage: UINT);
begin
  case AMessage of
    WM_LBUTTONUP,
    WM_LBUTTONDOWN,
...
var
  ...
  procedure ExtractChar(const AVirtualCode: Winapi.Windows.WPARAM; const ACharCode: Winapi.Windows.WPARAM;
                        const ALParam: Winapi.Windows.LPARAM;
                        var Key: Word; var Ch: WideChar; var Shift: TShiftState);
  begin
    if AVirtualCode = 0 then
    begin
      Ch := WideChar(ACharCode);
      Key := VkKeyScan(Ch);
    end
    else if ACharCode = 0 then
    begin
      Key := AVirtualCode;
      Ch := WideChar(AVirtualCode);
    end
    else
    begin
      Key := AVirtualCode;
    ...
    Shift := KeyDataToShiftState(ALParam);
    if Ch >= ' ' then
    begin
  ...
  function ExtractFormPoint(const AForm: TCommonCustomForm; const AMessage: TWMMouse): TPointF;
  var
    FormPixel: TSmallPoint;
  begin
    FormPixel := AMessage.Pos; // Form point in px
    Result := FormPxToDp(LForm, FormPixel); // dp
  end;
        ...
        case uMsg of
          ...
          WM_LBUTTONDOWN:
            begin
              ...
              FormPoint := ExtractFormPoint(LForm, TWMLButtonDown(Message));
              ScreenPoint := LForm.ClientToScreen(FormPoint); // dp
              LForm.MouseDown(TMouseButton.mbLeft, MouseToShiftState(wParam) + KeysToShiftState(wParam), FormPoint.X, FormPoint.Y);
          ...
          WM_LBUTTONUP:
            begin
              var FormInfo := PlatformWin.FormInfo[LForm];
              FormInfo.WasLeftMouseButtonPressed := False;
              WindowBorder := TWindowBorderWin(LForm.Border.WindowBorder);
              FormPoint := ExtractFormPoint(LForm, TWMLButtonUp(Message));
              ScreenPoint := LForm.ClientToScreen(FormPoint); // dp
              ...
              begin
                LastMousePos := ImpossibleMousePosition;
                var AdditionalMouseShiftState := FormInfo.AdditionalMouseShiftState;
                // We are extracting additional shif state value before invoking MouseUp, because form can change handle
                // in this event and FormInfo could be recreated.
                FormInfo.AdditionalMouseShiftState := [];
                LForm.MouseUp(TMouseButton.mbLeft, MouseToShiftState(wParam) + KeysToShiftState(wParam) +
                              AdditionalMouseShiftState + [ssLeft],
           ...
           WM_RBUTTONDBLCLK:
            begin
              LastMousePos := ImpossibleMousePosition;
              FormPoint := ExtractFormPoint(LForm, TWMMouse(Message));
              ScreenPoint := LForm.ClientToScreen(FormPoint); // dp
              Obj := LForm.ObjectAtPoint(ScreenPoint);
              if uMsg = WM_RBUTTONDBLCLK then
              begin
                var FormInfo := PlatformWin.FormInfo[LForm];
                FormInfo.AdditionalMouseShiftState := [ssDouble];
                LForm.MouseDown(TMouseButton.mbRight, MouseToShiftState(wParam) + KeysToShiftState(wParam) + FormInfo.AdditionalMouseShiftState,
                                FormPoint.X, FormPoint.Y);
              end
              else
                LForm.MouseDown(TMouseButton.mbRight, MouseToShiftState(wParam) + KeysToShiftState(wParam), FormPoint.X, FormPoint.Y);
            end;
          WM_MBUTTONDOWN:
            begin
              LastMousePos := ImpossibleMousePosition;
              FormPoint := ExtractFormPoint(LForm, TWMMButtonDown(Message));
              var FormInfo := PlatformWin.FormInfo[LForm];
              LForm.MouseDown(TMouseButton.mbMiddle, MouseToShiftState(wParam) + KeysToShiftState(wParam) + FormInfo.AdditionalMouseShiftState,
                              FormPoint.X, FormPoint.Y);
            end;
          WM_RBUTTONUP,
          WM_MBUTTONUP:
            begin
              ...
              begin
                var MouseButton := TMouseButton.Create(uMsg);
                LForm.MouseUp(MouseButton, MouseToShiftState(wParam) + KeysToShiftState(wParam) +
                              FormInfo.AdditionalMouseShiftState + MouseButton.ToShiftState, FormPoint.X, FormPoint.Y);
          ...
          WM_MBUTTONDBLCLK:
            begin
              var FormInfo := PlatformWin.FormInfo[LForm];
              FormInfo.WasLeftMouseButtonPressed := True;
              FormInfo.AdditionalMouseShiftState := [ssDouble];
              LastMousePos := ImpossibleMousePosition;
              FormPoint := ExtractFormPoint(LForm, TWMMouse(Message));
              var MouseButton := TMouseButton.Create(uMsg);
              LForm.MouseDown(MouseButton, MouseToShiftState(wParam) + KeysToShiftState(wParam) + FormInfo.AdditionalMouseShiftState,
                              FormPoint.X, FormPoint.Y);
            end;
          WM_MENUSELECT,
          WM_INITMENUPOPUP:
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    LastMousePos := P;
                    FormPoint := ExtractFormPoint(LForm, TWMMouseMove(Message));
                    LForm.MouseMove(MouseToShiftState(wParam) + KeysToShiftState(wParam), FormPoint.X, FormPoint.Y);
                  end;
                end;
              end
              else
              begin
                ...
                begin
                  ...
                  FormPoint := ExtractFormPoint(LForm, TWMMouseMove(Message));
                  var FormInfo := PlatformWin.FormInfo[LForm];
                  LForm.MouseMove(MouseToShiftState(wParam) + KeysToShiftState(wParam) + FormInfo.AdditionalMouseShiftState, FormPoint.X, FormPoint.Y);
                end;
              end;
          ...
          WM_SYSCHAR:
            begin
              // wParam includes Char code (not Virtual Code)
              ExtractChar(0, wParam, lParam, Key, Ch, Shift);
              var Handled := False;
              if LForm.Border.IsSupported then
              begin
                // Styled border doesn't support native MainMenu, so we emulate menu via TMenuBar component.
                // So we should ask to styled border handle this accelerator key first.
                WindowBorder := TWindowBorderWin(LForm.Border.WindowBorder);
                WindowBorder.KeyDown(Key, Ch, Shift, Handled);
              end;
          ...
          WM_SYSKEYDOWN:
            begin
              ...
              var Handled := False;
              LastKeyIsDeadKey := PeekMessage(Msg, hwnd, WM_SYSDEADCHAR, WM_SYSDEADCHAR, PM_NOREMOVE + PM_NOYIELD);
              if not LastKeyIsDeadKey and TranslateMessage(Msg) then
              begin
                WindowBorder := TWindowBorderWin(LForm.Border.WindowBorder);
                if not PeekMessage(Msg, hwnd, WM_SYSCHAR, WM_SYSCHAR, PM_REMOVE) then
                begin
                  // The message queue doesn't have detected accelerator key
                  Key := wParam;
                  Ch := #0;
          ...
          WM_KEYDOWN:
            begin
              ...
              if PeekMessage(Msg, hwnd, WM_DEADCHAR, WM_DEADCHAR, PM_NOREMOVE + PM_NOYIELD) then
              ...
              begin
                ...
                begin
                  ExtractChar(wParam, Msg.wParam, lParam, Key, Ch, Shift);
          ...
          WM_KEYUP:
            begin
              ...
              begin
                ...
                if TranslateMessage(Msg) and PeekMessage(Msg, hwnd, WM_CHAR, WM_CHAR, PM_REMOVE) then
                  ExtractChar(wParam, Msg.wParam, lParam, Key, Ch, Shift);
          ...
          WM_SYSKEYUP:
            begin
              if PlatformWin.FormInfo[LForm].WasLeftMouseButtonPressed then
                Result := 0
              else if (wParam = VK_MENU) or (wParam = VK_F10) then
              begin
                var MainMenu := TOpenForm(LForm).MainMenu;
                if LForm.Border.IsSupported and (MainMenu <> nil) then
                begin
                  // Form uses Styled Border. So it uses TMenuBar instead of native TMainMenu and we should initiate
                  // showing menu via passing WindowBorder as Root.
                  WindowBorder := TWindowBorderWin(LForm.Border.WindowBorder);
                  TMessageManager.DefaultManager.SendMessage(WindowBorder, TStartMenuLoopMessage.Create(WindowBorder), True);
                  Result := 0;
                end
...
begin
  Result := TWinWindowBorder.Create(AForm);
end;
...
begin
  ...
  begin
    ...
    if (WndParent = GetDesktopWindow) or (WndParent = 0) or AForm.IsPopupForm then
...
function TMouseButtonHelper.ToShiftState: TShiftState;
begin
  case Self of
    TMouseButton.mbLeft:
      Result := [ssLeft];
...
begin
  ...
  if AParameters and GCS_CURSORPOS <> 0 then
    ...
    // Native Korean IME Input system doesn't use cursor position. Instead of it it Highlights the last character in
...
{ TWinSystemAppearanceService }

constructor TWinSystemAppearanceService.Create;
begin
  inherited Create;
  FWndHandle := AllocateHWnd(WndProc);
end;

destructor TWinSystemAppearanceService.Destroy;
begin
  DeallocateHWnd(FWndHandle);
  inherited;
end;

function TWinSystemAppearanceService.GetSystemColor(const AType: TSystemColorType): TAlphaColor;
begin
  if FSystemColor[AType] = TAlphaColors.Null then
    FSystemColor[AType] := ReadSystemColorFromRegistry(AType);
  Result := FSystemColor[AType];
end;

function TWinSystemAppearanceService.GetSystemThemeKind: TSystemThemeKind;
begin
  if FSystemThemeKind = TSystemThemeKind.Unspecified then
    FSystemThemeKind := ReadThemeKindFromRegistry;
  Result := FSystemThemeKind;
end;
...
const
  ColorsKey = 'Software\Microsoft\Windows\DWM\'; // Do not localize
  AccentColorValue = 'AccentColor'; // Do not localize
var
  RegistryName: string;
begin
  Result := DefaultAccent;
  if TOSVersion.Check(10) then
  begin
    case AType of
      TSystemColorType.Accent:
        RegistryName := AccentColorValue;
    else
      RegistryName := AccentColorValue;
    end;
    ...
    try
      ...
      if LRegistry.KeyExists(ColorsKey) then
        if LRegistry.OpenKey(ColorsKey, False) then
        try
          if LRegistry.ValueExists(RegistryName) then
            Result := ColorToAlphaColor(TColor(LRegistry.ReadInt64(RegistryName)));
        finally
          LRegistry.CloseKey;
        end;
    finally
...
function TWinSystemAppearanceService.ReadThemeKindFromRegistry: TSystemThemeKind;
const
  ThemeKey = 'Software\Microsoft\Windows\CurrentVersion\Themes\Personalize\'; // Do not localize
  ThemeValue = 'AppsUseLightTheme'; // Do not localize
begin
  ...
  begin
    ...
    try
      LRegistry.RootKey := HKEY_CURRENT_USER;
      if LRegistry.KeyExists(ThemeKey) then
        if LRegistry.OpenKey(ThemeKey, False) then
        try
          if LRegistry.ValueExists(ThemeValue) and (LRegistry.ReadInteger(ThemeValue) <> 1) then
...
procedure TWinSystemAppearanceService.WMSettingChange(var AMessage: TWMSettingChange);
var
  NewThemekind: TSystemThemeKind;
  NewColors: TSystemColors;
  WasColorsChanged: Boolean;
begin
  if SameText('ImmersiveColorSet', string(AMessage.Section)) then // Do not localize
  begin
    NewThemekind := ReadThemeKindFromRegistry;
    WasColorsChanged := False;
    for var ColorType := Low(TSystemColorType) to High(TSystemColorType) do
    begin
      NewColors[ColorType] := ReadSystemColorFromRegistry(ColorType);
      if NewColors[ColorType] <> FSystemColor[ColorType] then
        WasColorsChanged := True;
    end;

    if (NewThemekind <> FSystemThemeKind) or WasColorsChanged then
    begin
      FSystemThemeKind := NewThemekind;
      FSystemColor := NewColors;
      TMessageManager.DefaultManager.SendMessage(nil, TSystemAppearanceChangedMessage.Create(TSystemAppearance.Create, True), True);
    end;
  end;
end;

procedure TWinSystemAppearanceService.WndProc(var AMessage: TMessage);
begin
  Dispatch(AMessage);
end;
...
initialization
  ...
  begin
    ...
    try
      FillRect(LDC, LForm.ClientRect.Round, BS);
    finally
      DeleteObject(BS);
    end;
  end;
